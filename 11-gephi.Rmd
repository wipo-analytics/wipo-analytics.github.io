...
The dataset can be downloaded from github [here]() [here](https://github.com/poldham/opensource-patent-analytics/blob/master/2_datasets/synbio_patents/synbio_inventors.xlsx?raw=true). 
...
Next you will be asked to select the file to use. We will use the [drones_fulltext_cleaned_5884.csv](INSERT) file that  is located on the WIPO Analytics website on GitHub.
...
Next we want to size the nodes. On the left, look for the Appearence tab and then with Nodes in grey choose the Ranking button. Here the minimum size is set to 20 and the maximum to 150. Note that the default setting is 10 and this is generally too small for easy visibility. Press Apply and you will see the network changes to display the size of nodes based on the frequency. You can always adjust the size of nodes later if you are unhappy with them. 
...
####![](images/figures_gephi/fig29_defaultcurve.png) TO DO
